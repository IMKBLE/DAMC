...
class MNISTEDGE_unpaired(data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None):
        ...
        if self.train:
            ...
            ##########################
            self.unparied_num = 100
            data_0 = scipy.io.loadmat('rand1/unpaired_a25003.mat')
            data_dict=dict(data_0)
            data_1 = data_dict['xsingle']
            data_2 = scipy.io.loadmat('rand1/unpaired_b25003.mat')
            data_dict=dict(data_2)
            data_3 = data_dict['ysingle']
            data_a = np.zeros((self.unparied_num, 1, 1750))
            data_b = np.zeros((self.unparied_num, 1, 79))
