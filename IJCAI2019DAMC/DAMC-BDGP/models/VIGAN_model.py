...
class VIGANModel(BaseModel):
    ...
    def initialize(self, opt):
        ...
        ###################################
        self.clu = networks.define_clustering(5,100,self.gpu_ids)
        ...
        self.netG_B = networks.define_G(opt.input_nB, opt.input_nA,
                                    opt.ngf, opt.which_model_netG, opt.norm, self.gpu_ids)
        self.AE = networks.define_AE(opt.input_nA, opt.input_nB, self.gpu_ids)
        #self.AE_B = networks.define_AE(128, 144, self.gpu_ids)
        #####################################
        if self.isTrain:
            use_sigmoid = opt.no_lsgan
            self.netD_A = networks.define_D(opt.input_nB, opt.ndf,
                                         opt.which_model_netD,
                                         opt.n_layers_D, use_sigmoid, self.gpu_ids)
            self.netD_B = networks.define_D(opt.input_nA, opt.ndf,
                                         opt.which_model_netD,
                                         opt.n_layers_D, use_sigmoid, self.gpu_ids)
    ...
    def set_input_train(self, images_a, images_b, target_i):
        input_A =images_a
        input_B =images_b
        #print('input_A:')
        #print(input_A)
...
#        self.shared = commonz
    ...
    def test_unpaired(self):
        self.real_A = Variable(self.input_A, volatile=True)
        self.fake_AA,self.fake_AB,self.com1 = self.AE.forward_a2b(self.real_A)

        self.real_B = Variable(self.input_B, volatile=True)
        self.fake_BA,self.fake_BB,self.com2 = self.AE.forward_b2a(self.real_B)
    ...
    def test_commonZ(self):
        ...
        # Autoencoder loss: fakeA
        self.commonZ = self.AE.forward_commonZ(self.real_A, self.real_B)
        # Autoencoder loss: fakeB
    ...
    def backward_AE_pretrain(self):
        # Autoencoder loss
        AErealA,self.fake_B,self.com1 = self.AE.forward_a2b(self.real_A)
        
        self.fake_A, AErealB,self.com2 = self.AE.forward_b2a(self.real_B)
        
        self.loss_AE_A = self.criterionAE(AErealA, self.real_A) + self.criterionAE(self.fake_B, self.real_B)
        self.loss_AE_B = self.criterionAE(self.fake_A, self.real_A) + self.criterionAE(AErealB, self.real_B)
        self.loss_AE_pre = self.loss_AE_A + self.loss_AE_B 
    ...
    def backward_AE(self):
        ...
        self.loss_AAE = self.criterionAE(AErealA, self.real_A) +self.criterionAE(AErealB, self.real_B)
        self.loss_GAE = self.criterionAE(self.fake_AA, self.real_A) + self.criterionAE(self.fake_BB, self.real_B)
        self.loss_AE = self.loss_AE + (self.loss_G_B + self.loss_G_A)\
                           +1*self.loss_GAE
...
#        AErealA, AErealB,self.com = self.AE.forward(self.real_A, self.real_B)
        ...
        self.fake_A, AErealB,self.com2 = self.AE.forward_b2a(self.real_B)
        self.loss_AE = self.criterionAE(AErealA, self.real_A) +self.criterionAE(AErealB, self.real_B)
        self.com = self.AE.forward_commonZ(self.real_A, self.real_B)
        # Clustering_loss
        self.q_i,self.q= self.clu.forward(self.com)
        self.loss_com =  self.commonloss(self.q_i,self.target_i.view(1,5))
#        
#        self.fake_AA,self.fake_B,self.com1 = self.AE.forward_a2b(self.real_A)
#        
#        self.fake_A,self.fake_BB,self.com2 = self.AE.forward_b2a(self.real_B)
...
#        self.loss_clustering2 =  self.commonloss(self.q_i2,self.target_i.view(1,10))
        common = torch.norm(self.com,p=1,dim=0)/2500
        self.loss_12 = torch.norm(common,p=2)/2500
        self.loss_clustering = 0.001*self.loss_12+self.loss_com + 1*self.loss_AE #+self.loss_clustering1 + self.loss_clustering2 
...
#        self.loss_G_B = self.criterionGAN(pred_fake, True)
#
#        self.loss_AE_all = self.criterionAE(self.fake_AA, self.real_A) + self.criterionAE(self.fake_BB, self.real_B)
#
#        self.loss_G = 5*self.loss_AE_all + (self.loss_G_A+self.loss_G_B)
#        
#        self.loss_G.backward()   
        ...
        self.loss_G_B = self.criterionGAN(pred_fake, True)
        # Forward cycle loss
        self.rec_A1,self.rec_B1,self.com22= self.AE.forward_b2a(self.fake_B)
        ...
        # Backward cycle loss
        self.rec_A2,self.rec_B2,self.com12 = self.AE.forward_a2b(self.fake_A)
        ...
        # combined loss
        self.loss_GAB = self.loss_G_A + self.loss_G_B
        self.loss_ae = self.criterionAE(self.fake_AA, self.real_A) +self.criterionAE(self.fake_BB, self.real_B)
        self.loss_G = self.loss_G_A + self.loss_G_B + 1*self.loss_ae + self.loss_cycle_A1 + self.loss_cycle_B2# + self.loss_idt_A + self.loss_idt_B
        self.loss_G.backward()
...
#        BE = self.loss_AE_pre.data[0]
        AE_D_A = self.loss_D_A.item()
        AE_G_A = self.loss_G_A.item()
#        Cyc_A = self.loss_cycle_A.data[0]
        AE_D_B = self.loss_D_B.item()
        AE_G_B = self.loss_G_B.item()
#        Cyc_B = self.loss_cycle_B.data[0]
        AE = self.loss_AAE.item()
        AE_GAE = self.loss_GAE.item()
#        CLU_loss = self.loss_clustering.data[0]
        ALL_loss = self.loss_AE.item()
        if self.opt.identity > 0.0:
            return OrderedDict([('AE', AE), ('BE', AE)])
        else:
            return OrderedDict([('D_A', AE_D_A), ('G_A', AE_G_A), ('AE', AE),('GAE', AE_GAE),
...
#        AE = self.loss_AE.data[0]
        ...
        loss12 = self.loss_12.item()
        if self.opt.identity > 0.0:
            return OrderedDict([('CLU', CLU_loss), ('AE', AE), ('12',loss12)])
        else:
            return OrderedDict([('CLU', CLU_loss), ('AE', AE), ('12',loss12)])
