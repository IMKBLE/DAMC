...
class AutoEncoder(nn.Module):
    def __init__(self, input_A, input_B):
        ...
        self.layer1_1 = nn.Linear(input_A, 500, bias = False)
        self.layer1_2 = nn.Linear(input_B, 500, bias = False)
        self.layer2 = nn.Linear(500, 300)
        self.layer3 = nn.Linear(300, 100)
        self.layer4 = nn.Linear(100, 300)
        self.layer5 = nn.Linear(300, 500)
#        self.layer5_2 = nn.Linear(300, 79)
    ...
    def forward(self, x1, x2):
        x1 = F.dropout(F.relu(self.layer1_1(x1.view(-1, 1750))), self.drop)
        x2 = F.dropout(F.relu(self.layer1_2(x2.view(-1, 79))), self.drop)
...
#        out2 = F.tanh(self.layer6_2(out2))
        out1 = out1.view(1,1,1750)
        out2 = out2.view(1,1,79)
        return out1, out2
    

    def forward_a2b(self, x1):
        x1 = F.dropout(F.relu(self.layer1_1(x1.view(-1, 1750))), self.drop)
        x1 = F.dropout(F.relu(self.layer2(x1)), self.drop)   
        self.com1  = F.dropout(F.relu(self.layer3(x1)), self.drop)        
...
#        out1 = F.sigmoid(self.layer6_1(x))
        out1 = torch.tanh(self.layer6_1(x))
#        out2 = F.relu(self.layer5_2(x))
#        out2 = F.sigmoid(self.layer6_2(x))
        out2 = torch.tanh(self.layer6_2(x))
        out1 = out1.view(1,1,1750)
        out2 = out2.view(1,1,79)
        return out1, out2, self.com1
    
    def forward_b2a(self, x2):
        x2 = F.dropout(F.relu(self.layer1_2(x2.view(-1, 79))), self.drop)
        x2 = F.dropout(F.relu(self.layer2(x2)), self.drop)           
        self.com2  = F.dropout(F.relu(self.layer3(x2)), self.drop)
...
#        out2 = F.sigmoid(self.layer6_2(x))
        ...
        out1 = out1.view(1,1,1750)
        out2 = out2.view(1,1,79)
        return out1, out2, self.com2
    
    def forward_commonZ(self, x1, x2):
        ...
        self.com = (self.beta1*x1+self.beta2*x2)/(self.beta1+self.beta2)
#        x = F.dropout(F.relu(self.layer2((x1+x2)/2)), self.drop)
#        self.com = F.dropout(F.relu(self.layer3(x)), self.drop)
#        self.com = F.dropout(F.relu(self.layer31(x)), self.drop)
        #x = F.dropout(F.relu(self.layer4(x)), self.drop)
